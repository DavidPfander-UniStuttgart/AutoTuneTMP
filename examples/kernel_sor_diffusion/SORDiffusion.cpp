...
// 9 * VEC_SIZE FLOPs
inline double_v DiffusionKernel(const double_v &di, const double_v &dt,
                                const double_v &db, const double_v &dl,
                                const double_v &dr) {
...
inline double SORResidualKernel(const double *red, const double *black,
                                const double *rhs) {
...
template <int line, int R> void RBSORUpdate(double *grid) {
...
// dimX*dimY*12  FLOPs
void RBSORCycle(double *grid_r, double *grid_b, const double *rhs_r,
                const double *rhs_b) {
#pragma omp parallel for num_threads(NUMTHREADS)
...
#pragma omp parallel for num_threads(NUMTHREADS)
  for (int y = 0; y < dimY; y += 2 * BLOCKSIZEY) {
    for (int x = 0; x < dimX / 2; x += BLOCKSIZEX * double_v::size()) {
      ...
      std::array<std::array<double_v, BLOCKSIZEX>, 2 * BLOCKSIZEY + 1> blk_r;
      for (int kx = 0; kx < BLOCKSIZEX; ++kx) {
        blk_l[2 * BLOCKSIZEY][kx] =
            double_v(&grid_r[idx + (2 * BLOCKSIZEY + 1) * dimX_half +
                             kx * double_v::size()],
                     VcEA);
      ...
      for (int ky = 0; ky < 2 * BLOCKSIZEY; ky += 2) {
        for (int kx = 0; kx < BLOCKSIZEX; ++kx) {
          red[ky][kx] = double_v(
              ...
              VcEA);
          rhs[ky][kx] = double_v(
              &rhs_b[idx + (ky + 1) * dimX_half + kx * double_v::size() + 1],
              VcEA);
          blk_l[ky][kx] = double_v(
              &grid_r[idx + (ky + 1) * dimX_half + kx * double_v::size()],
              VcEA);
          blk_r[ky + 1][kx] = double_v(
              &grid_r[idx + (ky + 1) * dimX_half + kx * double_v::size() + 1],
              VcEA);
          red[ky + 1][kx] = double_v(
              &grid_b[idx + (ky + 2) * dimX_half + kx * double_v::size()],
              VcEA);
          rhs[ky + 1][kx] = double_v(
              &rhs_b[idx + (ky + 2) * dimX_half + kx * double_v::size()], VcEA);
          blk_l[ky + 1][kx] = double_v(
              &grid_r[idx + (ky + 2) * dimX_half + kx * double_v::size()],
              VcEA);
          blk_r[ky + 2][kx] = double_v(
              &grid_r[idx + (ky + 2) * dimX_half + kx * double_v::size() + 1],
              VcEA);
        }
      }
      for (int ky = 0; ky < 2 * BLOCKSIZEY; ky += 2) {
        for (int kx = 0; kx < BLOCKSIZEX; ++kx) {
          rhs[ky][kx] -=
              DiffusionKernel(red[ky][kx], blk_r[ky + 2][kx], blk_r[ky][kx],
                              blk_l[ky][kx], blk_r[ky + 1][kx]);
          red[ky][kx] -= pre * rhs[ky][kx];
          rhs[ky + 1][kx] -=
              DiffusionKernel(red[ky + 1][kx], blk_l[ky + 2][kx], blk_l[ky][kx],
...
AUTOTUNE_EXPORT double SORDiffusion(std::vector<double> &grid_r,
                               ...
                               const double &eps, double &res, double &rate,
                               double &time, size_t &iter, size_t &itermax) {
  std::cout << OMEGA << ", " << BLOCKSIZEX << ", " << BLOCKSIZEY << ", "
            << NUMTHREADS << std::endl;
  ...
  while ((res = RBSORResidual(grid_r.data(), grid_b.data(), rhs_r.data(),
                              rhs_b.data())) > eps_limit) {
    if (res > res_top) {
      res_top = res;
      iter_top = iter;
    }
