...
size_t SORSolve(std::vector<double> &grid, const std::vector<double> &rhs,
  ...
  // autotune::countable_continuous_parameter p1("OMEGA", 1.9, 0.005, 1.8,
  // 1.995);
  ...
  autotune::SORDiffusion.set_parameter_values(parameters2);
  autotune::SORDiffusion.set_parameter_values(parameters);
  ...
  autotune::tuners::bisect_search btune(autotune::SORDiffusion, parameters2,
                                        10);
  btune.setup_test(test_result);
  autotune::SORDiffusion.set_parameter_values(btune.tune(
  ...
  for (int i = 0; i < 10; ++i) {
    autotune::SORDiffusion(grid_r, grid_b, rhs_r, rhs_b, eps, res, rate, time,
                           iter, itermax);
    ...
    std::cout << "Achieved " << double(SORFlops(iter)) * 1e-9 / time
              << " GFLOP/s" << std::endl;
  }
