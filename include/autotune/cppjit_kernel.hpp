...
private:
  ...
  void (*set_meta_pointer)(thread_meta); // , int64_t
...
public:
  ...
  // replaced
  // uses thread_local storage, call from thread calling "get_meta"!
  void set_meta(thread_meta meta) { // , int64_t thread_id
    if (!is_compiled()) {
      compile();
    }
    if (!set_meta_pointer) {
    ...
    set_meta_pointer(meta); // , thread_id
  };

  // // important: will forget data in case of a recompile
  // // due to data being stored within shared object which is unloaded and
  // // replaced
  void set_thread_id(size_t thread_id) {
    if (!is_compiled()) {
      compile();
    }
