...
class generalized_kernel<R, cppjit::detail::pack<Args...>>
 ...
 private:
  ...
  std::function<bool(parameter_value_set &)> valid_parameter_combination_functor;
  std::function<void(parameter_value_set &)> create_parameter_combination_functor;
  thread_meta meta;

 public:
  generalized_kernel(const std::string &kernel_name
                     //                     ,std::function<R(Args...)> &internal_kernel
                     )
      : abstract_kernel<R, cppjit::detail::pack<Args...>>(kernel_name)
  //        ,internal_kernel(internal_kernel)
  {
    valid_parameter_combination_functor = [](parameter_value_set &) { return true; };
  }

  void set_valid_parameter_combination_functor(
      std::function<bool(parameter_value_set &)> valid_parameter_combination_functor) {
    this->valid_parameter_combination_functor = valid_parameter_combination_functor;
  }
  ...
  void set_create_parameter_file_functor(
      std::function<void(parameter_value_set &)> create_parameter_combination_functor) {
    this->create_parameter_combination_functor = create_parameter_combination_functor;
  }
...
#define AUTOTUNE_DECLARE_GENERALIZED_KERNEL(kernel_signature, kernel_name)                  \
  namespace autotune {                                                                      \
  extern generalized_kernel<cppjit::detail::function_traits<kernel_signature>::return_type, \
...
#define AUTOTUNE_DEFINE_GENERALIZED_KERNEL(kernel_signature, kernel_name)            \
  namespace autotune {                                                               \
  generalized_kernel<cppjit::detail::function_traits<kernel_signature>::return_type, \
                     cppjit::detail::function_traits<kernel_signature>::args_type>   \
