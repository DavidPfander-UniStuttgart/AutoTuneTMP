...
template <size_t num_threads, size_t vector_width,
          template <class R, template <class... Args> class pack_type> class kernel_type>
  ...
  void operator()(Args... args) {
    ...
    std::function<void(grid_spec, thread_meta)> thread_wrapper =
        [this, &args...](grid_spec spec, thread_meta meta_base) {
          std::shared_ptr<kernel_type<void, cppjit::detail::pack<Args...>>> kernel_clone(
              kernel.clone());
          ...
          // if (std::shared_ptr<cppjit_kernel<void, cppjit::detail::pack<Args...>>> casted =
          //         std::dynamic_pointer_cast<cppjit_kernel<void, cppjit::detail::pack<Args...>>>(
          //             kernel_clone)) {
          ...
          //     std::dynamic_pointer_cast<generalized_kernel<void, cppjit::detail::pack<Args...>>>(
          //         kernel_clone);
