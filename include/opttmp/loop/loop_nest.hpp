...
execute_looped(const T (&)[dim], const T (&)[dim], const T (&)[dim],
...
template <size_t dim, size_t cur_dim, typename T, typename F>
typename std::enable_if<cur_dim != dim, void>::type
execute_looped(const T (&min)[dim], const T (&max)[dim], const T (&step)[dim],
...
template <size_t dim, size_t cur_dim, typename T, typename F>
typename std::enable_if<cur_dim == dim, void>::type
execute_looped(const std::array<T, dim> &, const std::array<T, dim> &,
               const std::array<T, dim> &, std::array<T, dim> &completed_index,
               F f) {
  f(completed_index);
}
...
template <size_t dim, size_t cur_dim, typename T, typename F>
typename std::enable_if<cur_dim != dim, void>::type
execute_looped(const std::array<T, dim> &min, const std::array<T, dim> &max,
               const std::array<T, dim> &step,
...
void loop_nest(const T (&min)[dim], const T (&max)[dim], const T (&step)[dim],
               F f) {
  T partial_index[dim];
