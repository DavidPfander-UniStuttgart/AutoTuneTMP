...
public:
  ...
  template <class Arc> void serialize(Arc &arc, const unsigned);
...
public:
  direction() = default;
  constexpr direction(integer j) : geo_type(j) {}
  ...
  direction flip() const { return direction(_count - i); }
  operator integer() const { return (i < 14) ? i : (i - 1); }
  integer flat_index() const { return (i < 14) ? i : (i - 1); }

  integer flat_index_with_center() const { return i; }

  static constexpr integer count() { return _count; }
...
public:
  ...
  integer operator[](const dimension &dim) const { return (i >> dim) & 1; }
...
inline side quadrant::get_side(integer d) const { return side((i >> d) & 1); }

constexpr geo_type::geo_type(integer j) : i(j) {}

constexpr side::side(integer i) : geo_type(i) {}

constexpr dimension::dimension(integer i) : geo_type(i) {}

constexpr face::face(integer i) : geo_type(i) {}

constexpr octant::octant(integer i) : geo_type(i) {}

constexpr quadrant::quadrant(integer i) : geo_type(i) {}

constexpr integer side::count() { return _count; }

constexpr std::array<side, side::count()> side::full_set() { return {{0, 1}}; }

constexpr integer dimension::count() { return _count; }

constexpr std::array<dimension, dimension::count()> dimension::full_set() {
  return {{0, 1, 2}};
}

constexpr integer face::count() { return _count; }

constexpr std::array<face, face::count()> face::full_set() {
  return {{0, 1, 2, 3, 4, 5}};
}

constexpr integer octant::count() { return _count; }

constexpr std::array<octant, octant::count()> octant::full_set() {
  return {{0, 1, 2, 3, 4, 5, 6, 7}};
}

constexpr integer quadrant::count() { return _count; }

constexpr std::array<quadrant, quadrant::count()> quadrant::full_set() {
  return {{0, 1, 2, 3}};
}

inline geo_type::operator integer() const { return i; }

inline face::face(const dimension &d, const side &s) { i = 2 * d.i + s.i; }

inline dimension face::get_dimension() const { return dimension(i / 2); }
...
inline side face::get_side() const { return side(i & 1); }

template <class Arc> void geo_type::serialize(Arc &arc, const unsigned) {
  arc &i;
}
...
inline side side::flip() const { return side(i ^ 1); }

inline face face::flip() const { return face(i ^ 1); }
}

constexpr integer INNER = 0;
constexpr integer OUTER = 1;

integer get_boundary_size(std::array<integer, NDIM> &,
                          std::array<integer, NDIM> &, const geo::direction &,
                          const geo::side &, integer inx, integer bw,
                          integer nx = -1);
