...
typedef std::pair<std::vector<multipole>, std::vector<space_vector>>
    multipole_pass_type;
typedef std::pair<std::vector<expansion>, std::vector<space_vector>>
    expansion_pass_type;
...
struct gravity_boundary_type {
  ...
  bool is_local;
  gravity_boundary_type() : M(nullptr), m(nullptr), x(nullptr) {}
  void allocate() {
    if (M == nullptr) {
...
struct interaction_type {
  ...
  space_vector x;
  // precomputed values: (-1.0/r, (i0-j0)/r^3, (i1-j1)/r^3, (i2-j2)/r^3), r -
  // distance(i - j)
  v4sd four;
...
struct boundary_interaction_type {
  ...
  boundary_interaction_type(const boundary_interaction_type &other) {
    second = other.second;
    first = other.first;
    four = other.four;
    x = other.x;
