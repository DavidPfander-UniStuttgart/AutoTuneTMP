...
template <typename... Ts> struct assign_potential_expansions_body_struct {
  ...
  assign_potential_expansions_body_struct(Ts &... args) : vars(args...) {}

  template <size_t i> void operator()() {
    // std::get<0> == potential_expansions_SoA
    // std::get<1> == cell_flat_index_unpadded
    // std::get<2> == cur_pot
    // std::get<3> == mask

    // m2m_vector tmp =
    //     potential_expansions_SoA.value<i>(cell_flat_index_unpadded) +
    //     cur_pot[i];
    // Vc::where(mask, tmp).memstore(
    //     potential_expansions_SoA.pointer<i>(cell_flat_index_unpadded),
    //     Vc::flags::element_aligned);
    m2m_vector tmp = std::get<0>(vars).template value<i>(std::get<1>(vars)) +
                     std::get<2>(vars)[i];
    Vc::where(std::get<3>(vars), tmp)
        .memstore(std::get<0>(vars).template pointer<i>(std::get<1>(vars)),
                  Vc::flags::element_aligned);
  }
};
...
void m2m_kernel::blocked_interaction_rho(
    struct_of_array_data<expansion, real, 20, ENTRIES, SOA_PADDING>
        &__restrict__ local_expansions_SoA,
    struct_of_array_data<space_vector, real, 3, ENTRIES, SOA_PADDING>
        &__restrict__ center_of_masses_SoA,
    struct_of_array_data<expansion, real, 20, ENTRIES, SOA_PADDING>
        &__restrict__ potential_expansions_SoA,
    struct_of_array_data<space_vector, real, 3, ENTRIES, SOA_PADDING>
        &__restrict__ angular_corrections_SoA,
  ...
  for (size_t inner_stencil_index = 0;
    ...
    m2m_int_vector theta_c_rec_squared_int =
        detail::distance_squared_reciprocal(cell_index_coarse,
    ...
    // struct test_struct {
    //   template <typename T> test_f() { return T(); }
    // };
    // test_struct test_instance;

    // assign_potential_expansions_body_struct assign_potential_expansions_body(
    //     potential_expansions_SoA, cell_flat_index_unpadded, cur_pot, mask);
    // opttmp::loop::unroll_loop_template<0, 10, 1>(
    //     assign_potential_expansions_body);
    ...
    tmp = potential_expansions_SoA.value<2>(cell_flat_index_unpadded) +
          cur_pot[2];
    Vc::where(mask, tmp).memstore(
        potential_expansions_SoA.pointer<2>(cell_flat_index_unpadded),
        Vc::flags::element_aligned);

    tmp = potential_expansions_SoA.value<3>(cell_flat_index_unpadded) +
          cur_pot[3];
    Vc::where(mask, tmp).memstore(
    ...
    tmp = potential_expansions_SoA.value<5>(cell_flat_index_unpadded) +
          cur_pot[5];
    Vc::where(mask, tmp).memstore(
        potential_expansions_SoA.pointer<5>(cell_flat_index_unpadded),
        Vc::flags::element_aligned);

    tmp = potential_expansions_SoA.value<6>(cell_flat_index_unpadded) +
          cur_pot[6];
    Vc::where(mask, tmp).memstore(
        potential_expansions_SoA.pointer<6>(cell_flat_index_unpadded),
        Vc::flags::element_aligned);

    tmp = potential_expansions_SoA.value<7>(cell_flat_index_unpadded) +
          cur_pot[7];
    Vc::where(mask, tmp).memstore(
        potential_expansions_SoA.pointer<7>(cell_flat_index_unpadded),
        Vc::flags::element_aligned);

    tmp = potential_expansions_SoA.value<8>(cell_flat_index_unpadded) +
          cur_pot[8];
    Vc::where(mask, tmp).memstore(
        potential_expansions_SoA.pointer<8>(cell_flat_index_unpadded),
        Vc::flags::element_aligned);

    tmp = potential_expansions_SoA.value<9>(cell_flat_index_unpadded) +
          cur_pot[9];
    Vc::where(mask, tmp).memstore(
    ...
    D_upper[0] = D_calculator.X[0] * D_calculator.X[0] * D_calculator.d3 +
                 2.0 * D_calculator.d2;
    ...
    // std::cout << "-----------------------" << std::endl;
    // std::cout << "n0_constant: " << n0_constant << std::endl;
    // std::cout << "m_partner[10]: " << m_partner[10] << std::endl;
    // std::cout << "local_expansions_SoA.value<10>(cell_flat_index): "
    //           << local_expansions_SoA.value<10>(cell_flat_index) <<
    //           std::endl;
    // std::cout << "n0_tmp: " << n0_tmp << std::endl;
    // // m_cell_iterator++; // 11
    // std::cout << "D_upper[0]: " << D_upper[0] << std::endl;
    // std::cout << "factor_sixth_v[10]: " << factor_sixth_v[10] << std::endl;
    ...
    // std::cout << "current_angular_correction[0]: "
    //           << current_angular_correction[0] << std::endl;
    // std::cout << "current_angular_correction[1]: "
    //           << current_angular_correction[1] << std::endl;
    // std::cout << "current_angular_correction[2]: "
    //           << current_angular_correction[2] << std::endl;
    ...
    D_upper[10] = D_calculator.X[1] * D_calculator.X[1] * D_calculator.d3 +
                  2.0 * D_calculator.d2;
    ...
    current_angular_correction[1] -=
        n0_tmp * (D_upper[10] * factor_sixth_v[16]);
    current_angular_correction[2] -=
        n0_tmp * (D_upper[11] * factor_sixth_v[16]);
    ...
    current_angular_correction[1] -=
        n0_tmp * (D_upper[11] * factor_sixth_v[17]);
    current_angular_correction[2] -=
        n0_tmp * (D_upper[12] * factor_sixth_v[17]);
    ...
    current_angular_correction[1] -=
        n0_tmp * (D_upper[12] * factor_sixth_v[18]);
    current_angular_correction[2] -=
        n0_tmp * (D_upper[13] * factor_sixth_v[18]);

    D_upper[14] = D_calculator.X[2] * D_calculator.X[2] * D_calculator.d3 +
                  2.0 * D_calculator.d2;
    ...
    current_angular_correction[1] -=
        n0_tmp * (D_upper[13] * factor_sixth_v[19]);
    current_angular_correction[2] -=
        n0_tmp * (D_upper[14] * factor_sixth_v[19]);
...
void m2m_kernel::blocked_interaction_non_rho(
    ...
    const size_t cell_flat_index_unpadded,
    const std::vector<multiindex<>> &stencil,
    const size_t outer_stencil_index) {
