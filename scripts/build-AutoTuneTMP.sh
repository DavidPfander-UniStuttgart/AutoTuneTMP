...
cmake -DBOOST_ROOT=$BOOST_ROOT -DCMAKE_INSTALL_PREFIX="$AUTOTUNETMP_ROOT" -DCPPJIT_ROOT="$CPPJIT_ROOT" -DVc_ROOT="$Vc_ROOT" -DCMAKE_C_COMPILER=$CC -DCMAKE_CXX_COMPILER=$CXX -DCMAKE_CXX_FLAGS=$CXX_FLAGS -DCMAKcd E_BUILD_TYPE=release -DWITH_LIKWID=ON ../
